...
- `POST /reprocess` runs synchronously in the request; fine for 8 users, not
  for 50,000.
- The LLM disambiguation stage has only been exercised against the
  deterministic stub in this environment (no `OPENAI_API_KEY` configured
  during development) — the real-call path is covered by mocked unit tests
  in `test_llm_disambiguate.py`, but hasn't been run against the live OpenAI
  API. Worth doing before trusting it in front of real ambiguous profiles.
